...
def get_pdf_path(payload):
    ...
    valid_paths = {resume["path"] for resume in list_resumes()}
...
def get_email_body_choice(payload):
    ...
    selected_template = (payload.get("htmlBody") or DEFAULT_HTML_BODY_PATH).strip()
    valid_paths = {template["path"] for template in list_email_templates()}
...
def get_excel_path(payload):
    source = (payload.get("receiverListType") or "default").strip().lower()
    if source == "default":
        return DEFAULT_EXCEL_PATH

    if source == "system":
        selected_excel = (payload.get("excelPath") or "").strip()
        ...
        if not excel_file.exists():
            raise ValueError(f"Receiver list not found: {selected_excel}")
...
def save_uploaded_excel(handler):
    ...
    original_name = Path(file_part.get_filename() or "").name
    ...
    with open(target_path, "wb") as out_file:
        out_file.write(file_part.get_payload(decode=True) or b"")
...
def run_send(payload):
    ...
    html_body_path, html_body_content = get_email_body_choice(payload)
    excel_path = get_excel_path(payload) if mode == "bulk" else DEFAULT_EXCEL_PATH
    ...
    try:
        if mode == "single":
            ...
            set_state(message=f"Single email finished for {name} <{email}>.")
        elif mode == "bulk":
            daily_limit = int(payload.get("limit") or DEFAULT_DAILY_LIMIT)
        ...
        set_state(message="Missing sender email or app password in email_sender.py.")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/resumes":
            self.send_json({"ok": True, "resumes": list_resumes(), "default": DEFAULT_PDF_PATH})
            return
        if self.path == "/api/email-bodies":
            self.send_json({"ok": True, "templates": list_email_templates(), "default": DEFAULT_HTML_BODY_PATH})
            return
        super().do_GET()
    ...
    def handle_send(self):
        ...
        current = get_state()
        if current["running"]:
            self.send_json({"ok": False, "message": "Email sending is already running."}, status=409)
            return

        stop_event.clear()
        set_state(running=True, mode=payload.get("mode"), message="Sending started.", events=[], last_event_id=0)
        send_thread = threading.Thread(target=run_send, args=(payload,), daemon=True)
        send_thread.start()
        self.send_json({"ok": True, **get_state()})

    def handle_upload_excel(self):
        try:
            uploaded_path = save_uploaded_excel(self)
            self.send_json({"ok": True, "path": uploaded_path})
        except Exception as exc:
            self.send_json({"ok": False, "message": str(exc)}, status=400)

    def handle_stop(self):
        if not get_state()["running"]:
            self.send_json({"ok": True, "message": "No email sending is running.", **get_state()})
            return

        stop_event.set()
        set_state(message="Stop requested. Waiting for the current email step to finish.")
        self.send_json({"ok": True, **get_state()})
...
if __name__ == "__main__":
    ...
    print(f"Email sender UI running at http://localhost:{PORT}")
